...
Purpose
-------
Read a ballistic cislunar release sweep, select cases, and optimize a fixed-time
four-impulse transfer correction sequence:
...
The decision variables are the Cartesian components of each impulsive Δv.

For four enabled maneuvers:

    4 burns x 3 components = 12 variables
...
Notes
-----
- This script keeps maneuver times fixed.
- It does not optimize launch/release geometry.
- It reconstructs the initial release state from ballistic sweep parameters.
- It can optionally include ΔV penalties, but for initial target convergence,
  leave the penalties disabled.
"""
...
def utc_to_jdtdb(utc_string):
    """
    Robust-ish UTC to JDTDB conversion.

    Handles strings like:
        2015-12-03 04:45:42.081531370

    by parsing with pandas and truncating to microsecond precision for SPICE.
    """
...
def build_tli_like_state_from_row(row, basis):
    """
    Reconstruct initial release state from one ballistic-sweep summary row.
    """
...
def get_enabled_maneuvers(config):
    ...
    # Sort by time fraction, just in case YAML order is accidental.
    ...
    if any(fractions[i] > fractions[i + 1] for i in range(len(fractions) - 1)):
        raise ValueError("Maneuver fractions must be nondecreasing.")

    if fractions[-1] > 1.0:
        raise ValueError(
            "This script assumes maneuvers occur within the transfer, "
            "with ACQ at fraction 1.0. Post-target trims are not propagated here."
        )
...
def maneuver_times_from_sequence(maneuvers, t_depart_jdtdb, t_target_jdtdb):
    ...
    for m in maneuvers:
        f = float(m["fraction"])
        times.append(float(t_depart_jdtdb) + f * tof)
...
def unpack_maneuver_vector(x, n_maneuvers):
    """
    Convert flat vector into list of 3D Δv vectors.

    x shape:
        [dv1_x, dv1_y, dv1_z, dv2_x, ...]
    """
    ...
    if len(x) != 3 * n_maneuvers:
        raise ValueError(
            f"Expected {3*n_maneuvers} decision variables, got {len(x)}."
        )
...
def select_cases_for_optimization(summary_df, config):
    ...
    if run_mode == "all":
        ...
        raise ValueError(f"Unknown four_burn_optimization.run_mode: {run_mode}")

    selected = selected.sort_values(ranking_col, ascending=ascending).reset_index(drop=True)

    print("Four-burn optimization case selection")
    print("-------------------------------------")
...
# =============================================================================
# Propagation with arbitrary enabled maneuver sequence
# =============================================================================
...
def propagate_with_maneuver_sequence(
    ...
    """
    Propagate with enabled maneuvers.

    Maneuvers are applied impulsively at maneuver_times.

    If a maneuver occurs at the target epoch, propagation to target happens first,
    then the final burn is applied.
    """
    ...
    for k, (t_burn, dv) in enumerate(zip(maneuver_times, dv_list)):
        ...
        if t_burn < current_t:
            raise ValueError("Maneuver times must be nondecreasing.")

        # Propagate to burn time if needed.
        if t_burn > current_t:
    ...
    # If last maneuver was before target, propagate to target.
...
def optimize_four_burn_for_case(
    ...
    maneuver_times = maneuver_times_from_sequence(
        maneuvers,
        t_depart_jdtdb,
        t_target_jdtdb,
    )
    ...
    target_success = bool(
        final_pos_err <= pos_tol and final_vel_err_mps <= vel_tol
    )
    ...
    for k, (m, t_burn, dv_k, dv_mag_mps) in enumerate(
        zip(maneuvers, maneuver_times, dv_list, dv_mags_mps)
    ):
        name = m["name"]
...
# =============================================================================
# Trajectory reconstruction for staged visualizations
# =============================================================================
...
def propagate_staged_trajectory(
    ...
    """
    Propagate the trajectory to target while applying only the first apply_count
    maneuvers. Remaining maneuvers are skipped.

    Examples:
        apply_count = 0: ballistic
        apply_count = 1: TCM-1 only
        apply_count = 2: TCM-1 + TCM-2
        apply_count = 3: TCM-1 + TCM-2 + TCM-3
        apply_count = 4: full sequence
    """
    ...
    # March through each maneuver epoch, whether or not the burn is applied.
    for k, t_burn in enumerate(maneuver_times):
        ...
        # Apply burn only if this burn is within staged prefix.
        if k < apply_count:
            state[3:] += dv_list[k]

    # Continue to target if needed.
    if current_t < float(t_target_jdtdb):
    ...
    t_all = np.hstack(all_times)
    X_all = np.vstack(all_states)
...
def save_staged_trajectories_for_case(
    ...
    state0 = build_tli_like_state_from_row(record, basis)
    maneuvers = get_enabled_maneuvers(config)
    maneuver_times = maneuver_times_from_sequence(
        maneuvers,
        t_depart_jdtdb,
    ...
    for label, apply_count in labels:
        ...
        filename = f"{case_label}_{label}_trajectory.csv"
    ...
    maneuver_df = pd.DataFrame(maneuver_rows)
    maneuver_path = traj_dir / f"{case_label}_maneuvers.csv"
    maneuver_df.to_csv(maneuver_path, index=False)
...
def plot_staged_trajectories(config, target_state, staged_paths, case_label):
    ...
    # Plot in a stable order: ballistic first, then staged burns.
...
def plot_representative_final_trajectories(config, target_state, representative_info):
    viz = config.get("visualization", {}).get("four_burn_representative_trajectories", {})
    ...
    for rep_name, info in representative_info.items():
        if not isinstance(info, dict):
            continue
        if "staged_paths" not in info:
            continue

        # final trajectory is the path with all enabled burns.
        staged_paths = info["staged_paths"]
        final_key = None
        for key in staged_paths.keys():
            if key != "ballistic" and key != "maneuver_csv":
                final_key = key

        if final_key is None:
            continue

        df = pd.read_csv(staged_paths[final_key])
        r = df[["x_geo_j2000_km", "y_geo_j2000_km", "z_geo_j2000_km"]].to_numpy()

        all_points.append(r)

        sample_id = info["sample_id"]
        dv_total = info["dv_total_mps"]

        ax.plot(
            r[:, 0],
            r[:, 1],
            r[:, 2],
            label=f"{rep_name}, id={sample_id}, ΔV={dv_total:.1f} m/s",
        )
    ...
    ax.set_title("Representative optimized four-burn trajectories")
    ax.legend(fontsize=7)

    set_axes_equal_3d(ax)
    plt.tight_layout()

    plot_path = out_dir / viz.get("filename", "four_burn_representative_trajectories.png")
...
def save_summary_outputs(config, opt_summary_df, metadata):
    out_dir = Path(config["four_burn_optimization"]["output_dir"])
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if bool(output_cfg.get("save_four_burn_summary_csv", True)):
        opt_summary_df.to_csv(summary_path, index=False)

    if bool(output_cfg.get("save_four_burn_metadata_json", True)):
...
def main():
    ...
    input_summary_csv = config["four_burn_optimization"]["input_summary_csv"]
    ...
    summary_path, metadata_path = save_summary_outputs(
        config=config,
        opt_summary_df=opt_summary_df,
        metadata=metadata,
    )

    # Representative staged trajectory saving and plotting
    representative_info = {}

    if bool(config.get("output", {}).get("save_representative_trajectories", True)):
        reps = choose_representatives(opt_summary_df, config)

        for rep_name, rep_record in reps.items():
            case_label = f"{rep_name}_sample_{int(rep_record['sample_id'])}"
            ...
            staged_plot = plot_staged_trajectories(
                config=config,
                target_state=target_state,
            ...
            representative_info[rep_name] = {
                "sample_id": int(rep_record["sample_id"]),
        ...
        final_plot = plot_representative_final_trajectories(
            config=config,
            target_state=target_state,
        ...
        metadata["representative_final_plot"] = final_plot

        out_dir = Path(config["four_burn_optimization"]["output_dir"])
        with open(out_dir / "four_burn_optimization_metadata.json", "w") as f:
            json.dump(metadata, f, indent=2)

    print()
    print("Four-burn optimization complete")
    print("-------------------------------")
