...
Dependencies
------------
Required: numpy, pandas, spiceypy, utilities.py,
          earth_moon_invisibility_zone.py
...
Run directly in PyCharm. The minimum working example uses the SPICE-based
Earth--Moon IV-zone implementation at one JD TDB epoch, creates a synthetic
residence CSV around that real cone geometry, runs a full optimization, then
verifies the optional previous-score shortcut on a second call.
"""
...
def build_single_epoch_search_geometry(
    ...
    """Build matched SPICE/IV-zone geometry for one optimizer epoch.

    This function calls ``earth_moon_invisibility_zone.py``, which directly
    furnishes ``de430.bsp`` and ``naif0012.tls``. The returned observer SECR
    position, IV-zone axis, fixed IV-zone half-angle, and ``FrameContext`` all
    use the same JD TDB epoch and Earth ephemeris.
    """
...
def _demo_snr_evaluator_factory(
    ...
    def evaluate(positions_synodic_km: FloatArray) -> FloatArray:
        ...
        # Deterministic range-only response for the optimization self-test.
        # The real workflow should replace this with PayloadSNRGridEvaluator.
        return np.asarray(
            16.0 * np.exp(-distance / 2.8e6),
            dtype=float,
        )
...
def minimum_working_example() -> None:
    """Run the optimizer using the SPICE-based Earth--Moon IV zone.

    The working directory must contain ``de430.bsp`` and ``naif0012.tls``.
    The demonstration places the observer approximately 1.5 million km from
    Earth toward the Sun at the selected JD TDB epoch, then constructs the
    IV-zone axis from the actual SPICE Earth and Moon directions.
    """
    ...
    # Furnish the same fixed kernels used by the IV-zone implementation, then
    # place the demonstration observer approximately at Sun--Earth L1.
    ...
    observer_position_geo_eme_km = (
        1.50e6 * sun_direction_geo_eme
    )

    # This call uses earth_moon_invisibility_zone.py rather than a hard-coded
    # cone. It also provides the matched observer SECR position and frame data.
    ...
    # Start close to the stronger synthetic residence lobe, measured relative
    # to the actual Earth--Moon IV-zone axis at this epoch.
    ...
    with TemporaryDirectory() as temporary_directory:
        ...
        # Second call: unchanged epoch and geometry, so the previous optimum
        # should pass the 95%-of-previous-score shortcut.
