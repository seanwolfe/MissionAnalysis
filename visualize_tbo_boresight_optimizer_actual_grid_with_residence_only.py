...
RESIDENCE_GAUSSIAN_SIGMA_VOXELS = 1.0
...
def evaluate_selected_boresight_slice(
    ...
    print(
        projection_name,
        "slice coordinate AU =",
        fixed_coordinate / AU_KM,
        "FOV pixels =",
        np.count_nonzero(fov_mask),
        "IV pixels =",
        np.count_nonzero(iv_mask),
    )
    ...
    if finite_snr.size:
        print(
            projection_name,
            "SNR range =",
            float(np.min(finite_snr)),
            float(np.max(finite_snr)),
        )
    else:
        print(projection_name, "has no finite SNR values")
...
def plot_residence_only_projection(
    projection_name: str,
    volume: dict[str, object],
) -> None:
    """Plot only the projected residence-time distribution for one view."""
    ...
    axis.set_aspect("equal", adjustable="box")
    axis.set_title(
        f"Projected residence-time distribution ({projection_name})"
    )
    figure.tight_layout()
    save_figure(figure, f"residence_only_{projection_name}")
...
def main() -> None:
    ...
    for projection_name in PROJECTIONS:
        print(f"Plotting residence-only projection for {projection_name}...")
        plot_residence_only_projection(
